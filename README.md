...
[官网](https://redmate.app) · [下载](https://redmate.app/download) · [使用文档](https://redmate.app/docs) · [English](README.en.md)
...
**RedMate** 是一款专为小红书（xiaohongshu / xhs / RED / Little Red Book）创作者与内容运营团队打造的 **macOS 原生桌面助手**。它把账号登录、图文/视频笔记发布、定时发布、批量管理这些日常重复劳动从浏览器搬进一个本地 App，并内置 **MCP（Model Context Protocol）Server**，让 Claude、Cursor、Cline 这类 AI 客户端可以直接接管你的小红书账号操作。
...
- ✍️ **图文 / 视频笔记发布** — 选图、写文案、加话题、@ 用户、定位、改封面，全部在 App 内完成
...
| 模块 | 能力 |
|---|---|
| **账号管理** | 扫码登录 · 多账号并行 · Cookie Keychain 加密 · 自动续期 |
| **笔记发布** | 图文笔记 · 视频笔记 · 话题 / @ / 定位 · 封面剪裁 · 草稿箱 |
| **定时发布** | 发布队列 · 后台保活 · 失败自动重试 · 关机续跑 |
| **MCP 接入** | HTTP SSE Server · 标准 MCP 协议 · 兼容 Claude / Cursor / Cline |
| **同步与协作** | 跨设备草稿同步 · 团队空间（Pro） |
...
> "看下我小红书今天有哪些新评论，挑三条值得回复的，用我的账号回上去。"
>
> "把这周三、五、日中午 12 点各排一篇护肤笔记。"
>
> "我刚拍的这组图配段文案发出去，话题用 #秋冬穿搭 #日杂风。"

当前可用的 MCP 工具：

- `xhs_list_accounts` — 列出已登录账号
- `xhs_get_active_account` — 当前活跃账号
- `xhs_set_active_account` — 切换账号
- `xhs_check_login` — 检查登录态
- `xhs_publish_note` — 发布图文笔记

更多工具（视频发布、定时发布、评论 / 私信管理、数据看板）正在陆续上线。
...
**Q：这是开源项目吗？**
A：不是。RedMate 是商业产品，本仓库只用于产品介绍与发布说明，源码不开放。
...
**Q：和命令行工具相比有什么优势？**
A：命令行工具适合开发者；RedMate 面向所有内容创作者，提供完整 GUI、定时调度、多账号管理、AI 协作等命令行不具备的能力。
